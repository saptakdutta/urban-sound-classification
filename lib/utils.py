...
def plot_images(
    images, nrows=None, ncols=None, figsize=None, ax=None, axis_style="on", bgr2rgb=True
):
    """
    ...
    if not isinstance(images, (list, np.ndarray)):
        raise AttributeError(
            "The images parameter should be a list of images, "
...
def drawProgressBar(current, total, string="", barLen=20):
    ...
    sys.stdout.write(
        "Progress: [{:<{}}] {}/{}".format(
            "=" * int(barLen * percent) + arrow, barLen, current, total
        )
        + string
    )
    sys.stdout.flush()
